...
final class Fold {
	...
	synchronized double[] next_strip() {
		...
		if (debug)
			System.out.println("Fold.next_strip called");
		if (level == stop) {
	...
	void x_update(double scale, double mix, double[] a, double[] b, double[] c) {
		...
		if (mix <= 0.0) {
			...
			for (i = 0; i < count - 2; i += 2) {
				mp[i + 1] = 0.25 * (lp[i] + rp[i] + lp[i + 2] + rp[i + 2]) + (scale * p.r.nextGaussian());
			}
			/* }}} */
		} else if (mix >= 1.0) {
			// random offset to old value
			for (i = 0; i < count - 2; i += 2) {
				mp[i + 1] = mp[i + 1] + (scale * p.r.nextGaussian());
			}
		} else {
			// mixed update
			for (i = 0; i < count - 2; i += 2) {
	...
	void p_update(double scale, double mix, double[] a, double[] b, double[] c) {
		...
		if (mix <= 0.0) {
			// random offset to average of new points
			for (i = 0; i < count - 2; i += 2) {
				mp[i + 1] = 0.25 * (lp[i + 1] + rp[i + 1] + mp[i] + mp[i + 2]) + (scale * p.r.nextGaussian());
			}
		} else if (mix >= 1.0) {
	...
	void t_update(double scale, double mix, double[] a, double[] b, double[] c) {
		...
		if (mix <= 0.0) {
			...
			mp[0] = THIRD * (lp[0] + rp[0] + mp[1]) + (scale * p.r.nextGaussian());
			for (i = 1; i < count - 3; i += 2) {
				mp[i + 1] = 0.25 * (lp[i + 1] + rp[i + 1] + mp[i] + mp[i + 2]) + (scale * p.r.nextGaussian());
			}
			mp[i + 1] = THIRD * (lp[i + 1] + rp[i + 1] + mp[i]) + (scale * p.r.nextGaussian());

		} else if (mix >= 1.0) {
			// random offset to old values
			for (i = 0; i < count; i += 2) {
				mp[i] = mp[i] + (scale * p.r.nextGaussian());
			}
		} else {
			// mixed update
			mp[0] = (mix * mp[0]) + we * (lp[0] + rp[0] + mp[1]) + (scale * p.r.nextGaussian());
			for (i = 1; i < count - 3; i += 2) {
				mp[i + 1] = (mix * mp[i + 1]) + w * (lp[i + 1] + rp[i + 1] + mp[i] + mp[i + 2])
						+ (scale * p.r.nextGaussian());
			}
			mp[i + 1] = (mix * mp[i + 1]) + we * (lp[i + 1] + rp[i + 1] + mp[i]) + (scale * p.r.nextGaussian());
		}
	}
	...
	void v_update(double scale, double mix, double[] a, double[] b, double[] c) {
		...
		if (mix <= 0.0) {
			...
			mp[0] = 0.5 * (lp[1] + rp[1]) + (scale * p.r.nextGaussian());
			for (i = 1; i < count - 3; i += 2) {
				mp[i + 1] = 0.25 * (lp[i] + rp[i] + lp[i + 2] + rp[i + 2]) + (scale * p.r.nextGaussian());
			}
			mp[i + 1] = 0.5 * (lp[i] + rp[i]) + (scale * p.r.nextGaussian());
		} else if (mix >= 1.0) {
			// random offset to old values
			for (i = 0; i < count; i += 2) {
				mp[i] = mp[i] + (scale * p.r.nextGaussian());
			...
			// mixed update
			mp[0] = (mix * mp[0]) + we * (lp[1] + rp[1]) + (scale * p.r.nextGaussian());
			for (i = 1; i < count - 3; i += 2) {
				mp[i + 1] = (mix * mp[i + 1]) + w * (lp[i] + rp[i] + lp[i + 2] + rp[i + 2])
						+ (scale * p.r.nextGaussian());
			}
			mp[i + 1] = (mix * mp[i + 1]) + we * (lp[i] + rp[i]) + (scale * p.r.nextGaussian());
		}
	}
	...
	void hside_update(double scale, double mix, double[] a, double[] b, double[] c) {
		...
		if (mix <= 0.0) {
			// random offset to average of new points
			for (i = 0; i < count; i += 2) {
				mp[i] = 0.5 * (lp[i] + rp[i]) + (scale * p.r.nextGaussian());
			}
		} else if (mix >= 1.0) {
			...
			// mixed update
			for (i = 0; i < count; i += 2) {
				mp[i] = (mix * mp[i]) + w * (lp[i] + rp[i]) + (scale * p.r.nextGaussian());
			}
		}
	}
	...
	void vside_update(double scale, double mix, double[] a) {
		...
		if (mix <= 0.0) {
			// random offset to average of new points
			for (i = 0; i < count - 2; i += 2) {
				mp[i + 1] = 0.5 * (mp[i] + mp[i + 2]) + (scale * p.r.nextGaussian());
			}
		} else if (mix >= 1.0) {
			// random offset to old values
			for (i = 0; i < count - 2; i += 2) {
			...
			// mixed update
			for (i = 0; i < count - 2; i += 2) {
				mp[i + 1] = (mix * mp[i + 1]) + w * (mp[i] + mp[i + 2]) + (scale * p.r.nextGaussian());
			}
		}
	}
	...
	double[] random_strip() {
		...
		int i;
		if (debug)
			System.out.println("Fold.random_strip called");
		for (i = 0; i < count; i++) {
